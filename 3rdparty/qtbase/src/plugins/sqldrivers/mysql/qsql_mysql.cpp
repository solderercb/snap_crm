...
public:
    enum RealConnState {Offline = 0, Reconnected, Online};
...
// error may be occured due to connection loss; checking server and reconnecting
int QMYSQLResultPrivate::realConnectionState()
{
    int r = 0;
    r = mysql_ping(drv_d_func()->mysql);
    if (r == 0) { // server online or reconnection was success
        // mysql_thread_id() not recommended to use, see https://dev.mysql.com/doc/c-api/5.7/en/mysql-thread-id.html
        int newThreadId = mysql_thread_id(drv_d_func()->mysql);
        if(threadId != newThreadId)
        {
            threadId = newThreadId;
            return RealConnState::Reconnected;
        }
        else
        {
            return RealConnState::Online;
        }
    }

    return RealConnState::Offline;
}
...
int QMYSQLResult::exec(const QString &query)
{
    Q_D(QMYSQLResult);

    const QByteArray encQuery(fromUnicode(d->drv_d_func()->tc, query));
    return mysql_real_query(d->drv_d_func()->mysql, encQuery.data(), encQuery.length());
}

void QMYSQLResult::setOfflineError()
{
    Q_D(QMYSQLResult);
    setLastError(qMakeError(QLatin1String("server has gone away"),
                            QSqlError::ConnectionError, d->drv_d_func()));
}
...
bool QMYSQLResult::reset (const QString& query)
{
    ...
    if (exec(query)) {
        ...
        switch (d->realConnectionState())
        {
            case QMYSQLResultPrivate::RealConnState::Reconnected:
                if (exec(query) == 0) { // try to exec query after reconnection
                    break;
                } else {
                    return false;
                }
            case QMYSQLResultPrivate::RealConnState::Offline: setOfflineError(); return false;
            case QMYSQLResultPrivate::RealConnState::Online: // other reason of query exec error
...
bool QMYSQLResult::prepare(const QString& query)
{
    ...
    // mysql_thread_id() not recommended to use, see https://dev.mysql.com/doc/c-api/5.7/en/mysql-thread-id.html
    if(d->threadId == 0)
         d->threadId = mysql_thread_id(d->drv_d_func()->mysql);
    ...
    if (r != 0) {
        ...
        {
            case QMYSQLResultPrivate::RealConnState::Reconnected:
                if (mysql_stmt_prepare(d->stmt, encQuery.constData(), encQuery.length()) == 0) {   // try to prepare query after reconnection
                    break;
                } else {
                    cleanup();
                    return false;
                }
            case QMYSQLResultPrivate::RealConnState::Offline: {
                    setOfflineError();
                    // if stmt prepare failed due to connection lost then lastQuery() must return empty string, otherwise after reconnect stmt will not be prepared and exec will fail
                    d->sql.clear();
                    /*cleanup();*/
                    return false;
                }
            case QMYSQLResultPrivate::RealConnState::Online: // other reason of stmt prepare error (server online, but thread ID not changed)
...
bool QMYSQLResult::exec()
{
    ...
    if (r != 0) {
        ...
        switch (d->realConnectionState())
        {
            case QMYSQLResultPrivate::RealConnState::Reconnected: this->prepare(d->sql); break; // stmt must be prepaired
            case QMYSQLResultPrivate::RealConnState::Offline: setOfflineError(); return false;
            case QMYSQLResultPrivate::RealConnState::Online: // other reason of stmt reset error (server online, but thread ID not changed)
            default: return false;
        }
    }
    ...
    if (r != 0) {
        ...
        int cs = d->realConnectionState();
        switch (cs)
        {
            case QMYSQLResultPrivate::RealConnState::Reconnected:
                if (mysql_stmt_execute(d->stmt) == 0) {   // try to exec query after reconnection
                    break;
                } else {
                    return false;
                }
