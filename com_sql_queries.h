...
#define QUERY_SEL_DOC_TEMPLATES_LIST        QString("SELECT  `description`, `name`, `id`, `checksum`FROM `doc_templates` WHERE `app` = 'SNAP';")
...
#define QUERY_UPD_DOC_TEMPL_DATA(doc, data, hash)  QString("UPDATE `doc_templates` SET `data` = UNHEX('%1'), `checksum` = UNHEX('%2') WHERE `name` = '%3' AND `app` = 'SNAP';").arg((data)).arg((hash)).arg((doc))
