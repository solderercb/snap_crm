...
void SPrintPOSReport::addPrintJob(SRepairModel *repair)
{
    if(printer)
    {
        ...
        switch(repair->stateId())
        {
            ...
            case Global::RepStateIds::ReturnedInCredit: BOQModel = new WorkshopSaleModel(); BOQModel->loadTable(repair->id()); break;
            default: ;
        }
        ...
        *printer << EscPosQt::EscPosPrinter::JustificationLeft
          << EscPosQt::EscPosPrinter::formatAs2CTable("ДАТА ПРИЁМА", repair->createdStr());

        *printer << EscPosQt::EscPosPrinter::formatAs2CTable("КЛИЕНТ", m_client->fullShortName().toString())
        ...
        *printer << EscPosQt::EscPosPrinter::PrintModes();
        ...
        if (BOQModel)
        {
            ...
            *printer << EscPosQt::EscPosPrinter::PrintModes()
            ...
            {
                index = BOQModel->index(i, SSaleTableModel::Columns::Name);
                *printer << EscPosQt::EscPosPrinter::formatAs2CTable(
                                index.data().toString(),
                                QString("x%1  =%2") \
                                    .arg(index.siblingAtColumn(SSaleTableModel::Columns::Count).data().toInt()) \
                                    .arg(index.siblingAtColumn(SSaleTableModel::Columns::Amount).data().toString()));
            }
